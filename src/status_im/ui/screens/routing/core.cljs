...
(defn app-init-progress []
  (fn []
    (let [this        (reagent/current-component)
          ...
          navigate-to (get-in props [:navigation :navigate])]
      ;; NOTE: During the first navigate-to dispatch the naviagor is not initialized and event does not execute.
      ;; this little hack repeats the latest navigate event after initialization.
      (when-let [view-id @(re-frame/subscribe [:view-id])]
        (navigate-to (name view-id)))
      [progress/progress])))
